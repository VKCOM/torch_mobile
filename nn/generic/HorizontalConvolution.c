...
static int nn_(HorizontalConvolution_updateOutput)(lua_State *L)
{
...
static int nn_(HorizontalConvolution_updateGradInput)(lua_State *L)
{
   THTensor *input = luaT_checkudata(L, 2, torch_Tensor);
...
static int nn_(HorizontalConvolution_accGradParameters)(lua_State *L)
{
   THTensor *input = luaT_checkudata(L, 2, torch_Tensor);
   THTensor *gradOutput = luaT_checkudata(L, 3, torch_Tensor);
...
static const struct luaL_Reg nn_(HorizontalConvolution__) [] = {
  {"HorizontalConvolution_updateOutput", nn_(HorizontalConvolution_updateOutput)},
  {"HorizontalConvolution_updateGradInput", nn_(HorizontalConvolution_updateGradInput)},
  {"HorizontalConvolution_accGradParameters", nn_(HorizontalConvolution_accGradParameters)},
  {NULL, NULL}
};


static void nn_(HorizontalConvolution_init)(lua_State *L)
{
  luaT_pushmetatable(L, torch_Tensor);
  luaT_registeratname(L, nn_(HorizontalConvolution__), "nn");
  lua_pop(L,1);
}
